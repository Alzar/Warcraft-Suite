<?php
/**
 * @brief		Battle.net Login Handler
...
namespace IPS\wowsuite\Login\Handler\OAuth2;
...
/**
 * Battle.net Login Handler
 */
class _BattleNet extends \IPS\Login\Handler\OAuth2
{
    /**
     * Get title
     *
    ...
    {
        return 'login_handler_BattleNet';
    }
    ...
    public function acpForm()
    {
        ...
        \IPS\Member::loggedIn()->language()->words['oauth_client_id'] = \IPS\Member::loggedIn()->language()->addToStack('login_bnet_client');
        \IPS\Member::loggedIn()->language()->words['oauth_client_client_secret'] = \IPS\Member::loggedIn()->language()->addToStack('login_bnet_secret');

        return array_merge(
            array(
                'battle_tag'	=> new \IPS\Helpers\Form\Radio( 'login_battle_tag', isset( $this->settings['battle_tag'] ) ? $this->settings['battle_tag'] : 1, FALSE, array(
                    'options' => array(
                        1			=> 'login_battle_tag_bnet',
                        0			=> 'login_battle_tag_disabled',
                    ),
                    'toggles' => array(
                        1			=> array( 'login_update_name_changes_inc_optional' ),
                    )
                ), NULL, NULL, NULL, 'login_battle_tag' ),
            ),
            parent::acpForm()
        );
    ...
    {
        return \IPS\Theme::i()->resource( 'logos/login/BattleNet.png', 'bnet', 'interface' );
    }
    ...
    {
        return \IPS\Http\Url::external('https://us.battle.net/oauth/authorize');
    }
    ...
    {
        return \IPS\Http\Url::external('https://us.battle.net/oauth/token');
    }
    ...
    {
        if ( isset( $this->settings['battle_tag'] ) and $this->settings['battle_tag'] )
        {
            return $this->_userData( $accessToken )['battletag'];
        }
        return NULL;
    }
    ...
    public function userProfileName( \IPS\Member $member )
    {
        ...
        return $this->_userData( $link['token_access_token'] )['battletag'];
    }
    ...
    public function syncOptions( \IPS\Member $member, $defaultOnly = FALSE )
    {
        ...
        if ( isset( $this->settings['update_name_changes'] ) and $this->settings['update_name_changes'] === 'optional' and isset( $this->settings['battle_tag'] ) and $this->settings['battle_tag'] )
        {
            $return[] = 'name';
        }
    ...
    protected function _userData( $accessToken )
    {
        ...
        {
            $response = \IPS\Http\Url::external( "https://us.battle.net/oauth/userinfo" )
                ->setQueryString(
                    array(
